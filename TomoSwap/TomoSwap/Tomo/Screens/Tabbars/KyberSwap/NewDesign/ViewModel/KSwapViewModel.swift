...
class KSwapViewModel {
  ...
  let knc = KNSupportedTokenStorage.shared.defaultToken
  ...
  var isCapEnough: Bool {
    let ethAmount: BigInt = {
      if self.from.isTOMO { return self.amountFromBigInt }
      if self.to.isTOMO { return self.amountToBigInt }
      let ethRate: BigInt = {
        let cacheRate = KNRateCoordinator.shared.ethRate(for: self.from)
        return cacheRate?.rate ?? BigInt(0)
      }()
  ...
  var isTOMOSwapAmountAndFeeTooBig: Bool {
    if !self.from.isTOMO { return false } // not ETH
  ...
  var isHavingEnoughETHForFee: Bool {
    ...
    if self.from.isTOMO { fee += self.amountFromBigInt }
