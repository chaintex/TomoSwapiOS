...
class KNTokenStorage {
  ...
  var ethToken: TokenObject {
   return self.tokens.first(where: { $0.isTOMO }) ?? KNSupportedTokenStorage.shared.tomoToken
  }

  var kncToken: TokenObject {
    return self.tokens.first(where: { $0.isCTT }) ?? KNSupportedTokenStorage.shared.defaultToken
  }
