...
struct RealmConfiguration {
    ...
    static func globalConfiguration(for chainID: Int = KNEnvironment.default.chainID) -> Realm.Configuration {
      var config = Realm.Configuration()
      config.fileURL = config.fileURL!.deletingLastPathComponent().appendingPathComponent("tomoswap-global-\(chainID).realm")
      return config
    }

  static func kyberGOConfiguration(for userID: Int, chainID: Int = KNEnvironment.default.chainID) -> Realm.Configuration {
      var config = Realm.Configuration()
      config.fileURL = config.fileURL!.deletingLastPathComponent().appendingPathComponent("tomoswap-kybergo-\(userID)-\(chainID).realm")
      return config
    }
}
